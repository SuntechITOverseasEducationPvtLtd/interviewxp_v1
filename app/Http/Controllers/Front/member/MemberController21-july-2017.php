...
class MemberController extends Controller
{
    							...
    							)
    {
        ...
        $obj_member_id   = $this->MemberDetailModel->where('user_id',$this->user_id)->first(['id']);
    ...
    public function post_interview()
    {
        ...
        $arr_specialization = [];
        $obj_specialization = $this->SpecializationModel->where('is_active','=',1)->get();
...
*/
   ...
   public function delete_reference_book($reference_id)
    ...
    {                    
        ...
        $countMaxSchedules = $this->TrainingSchedulesModel->where(['member_id'=>$arr_data['member_id']])->count();
        if($countMaxSchedules > 1)
        {
            /*Flash::error('Warring! you already reached maximum number of schedules.');
            return redirect('member/scheduleClass');*/
            return Response::json([
                    'message'=>'Warring! you already reached maximum number of schedules.',
                    'status' =>false
                ]);
    ...
    public function edit_schedule($id)
    {
        ...
        $obj_schedule = $this->TrainingSchedulesModel->where('id',$id)->first();
        $arr_schedule=[];                        
        if($obj_schedule)
        {
    ...
    {                    
        ...
        $result = $this->TrainingSchedulesModel->where(['id'=>$id])->update($arr_data);
        if($result)
        { 

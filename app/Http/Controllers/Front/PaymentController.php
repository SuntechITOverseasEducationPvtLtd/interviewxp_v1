...
class PaymentController extends Controller
{
    public function __construct(PriceListModel $price_list,
                                ...
                                MultiReferenceBookModel $ref_book_model,
                                ManageEmailModel $manage_email)
    {
        ...
        $this->MultiReferenceBookModel     = $ref_book_model;
        $this->arr_view_data        = [];
    ...
    {		
        ...
        $notification_request = $request->input('notification_request');
		$selected_companies_price = '';
		if(isset($request->input('radio_comapny')['price_for_5_companies']))
		{
			$selected_companies_price = $request->input('radio_comapny')['price_for_5_companies'];
		}
		else if(isset($request->input('radio_comapny')['price_for_10_companies']))
		{
			$selected_companies_price = $request->input('radio_comapny')['price_for_10_companies'];
		}
		else if(isset($request->input('radio_comapny')['price_for_20_companies']))
		{
			$selected_companies_price = $request->input('radio_comapny')['price_for_20_companies'];
		}
        ...
        if(isset($arr_price_list))
        {
           ...
           $price_for_5_companies      = $arr_price_list['price_for_5_companies'];
           $price_for_10_companies      = $arr_price_list['price_for_10_companies'];
           $price_for_20_companies      = $arr_price_list['price_for_20_companies'];
		...
		else if(!empty($selected_companies_price))
		{
			if($count == 20)
			{
				$total_interview_amount = $price_for_20_companies;
			}
			else if($count == 10)
			{
				$total_interview_amount = $price_for_10_companies;
			}
			else{
				$total_interview_amount = $price_for_5_companies;
			}
       ...
       // {
        ...
        $this->arr_view_data['selected_companies_price']           = $selected_companies_price;
        ...
        $this->arr_view_data['notification_request']  = $notification_request;
    ...
    public function pay_now(Request $request)
    {
	  ...
	  $account_settings = $this->AccountSettingModel->select('tds','admin_commission')->first();
	  $admin_commission = $account_settings->admin_commission;
	  $member_commission = 100-$account_settings->admin_commission;
	  $tds_percent = $account_settings->tds;
	  
	  $member_amount = ($member_commission * $decrypt_data['sub_total'])/100;
	  $admin_amount = ($admin_commission * $decrypt_data['sub_total'])/100;
	  

      $arr_transaction                   = [];
      $arr_transaction['base_price']      = $decrypt_data['sub_total'];
      $arr_transaction['grand_total']      = $decrypt_data['grand_total'];
      $arr_transaction['admin_amount']     = $admin_amount;
      $arr_transaction['member_amount']    = $member_amount;
      $arr_transaction['tds_percent']    = $tds_percent;
      $arr_transaction['admin_commission']    = $admin_commission;
      ...
      $arr_transaction['notification_request']   = $decrypt_data['notification_request'];
      //dd($arr_transaction);
      ...
      {  
          ...
          if($decrypt_data['TextResumeType'] == 1)
          {
            ...
            if(!empty($decrypt_data['coach_reference_book']) || !empty($decrypt_data['coach_realissues']) || !empty($decrypt_data['coach_company']))
            {
              ...
              $coach['cgst'] = 0;
              $coach['sgst'] = 0;
			  $coach['member_commission'] = 0;
			  $coach['admin_commission'] = 0;

              if(!empty($decrypt_data['coach_reference_book']))
                $coach['combo_type'] = 'Coach+Qa';
              elseif(!empty($decrypt_data['N']))
            ...
            else
            {
			  ...
			  $coach['member_commission'] = ($member_commission * $decrypt_data['validityResume'])/100;
			  $coach['admin_commission'] = ($admin_commission * $decrypt_data['validityResume'])/100; 	
            }     
            
            $transactionHistory[] = $coach;
          }

          if(!empty($decrypt_data['reference_book_bought']) || !empty($decrypt_data['combo_coach_interview_qa_price']))
          {
            ...
            if(!empty($decrypt_data['coach_reference_book']))
            {
			  ...
			  $interview_qa['member_commission'] = ($member_commission * $decrypt_data['combo_coach_interview_qa_price'])/100;
			  $interview_qa['admin_commission'] = ($admin_commission * $decrypt_data['combo_coach_interview_qa_price'])/100;
            }
            else
            {
			  ...
			  $interview_qa['member_commission'] = ($member_commission * $decrypt_data['reference_book_bought'])/100;
			  $interview_qa['admin_commission'] = ($admin_commission * $decrypt_data['reference_book_bought'])/100; 	
            }            
          ...
          if(count($decrypt_data['arr_ticket_name']) > 0)
          {
            ...
            if(!empty($decrypt_data['coach_realissues']))
            {
			  ...
			  $workexp['member_commission'] = ($member_commission * $decrypt_data['combo_coach_realissues_price'])/100;
			  $workexp['admin_commission'] = ($admin_commission * $decrypt_data['combo_coach_realissues_price'])/100;
            }
            else
            {
			  ...
			  $workexp['member_commission'] = ($member_commission * $decrypt_data['arr_ticket_name'][0]['ticket_price'])/100;
			  $workexp['admin_commission'] = ($admin_commission * $decrypt_data['arr_ticket_name'][0]['ticket_price'])/100;	
            }
          ...
          if(count($decrypt_data['company_ids']) > 0)
          {
            foreach($decrypt_data['company_ids'] as $key => $id)
            {
              ...
              if(!empty($decrypt_data['coach_company']))
              {
                ...
                $company['igst'] = (!empty($decrypt_data['igst']) && $key == 0) ? $decrypt_data['combo_coach_company_price'] * ($decrypt_data['igst']/100) : 0;
                $company['cgst'] = (!empty($decrypt_data['cgst']) && $key == 0) ? $decrypt_data['combo_coach_company_price'] * ($decrypt_data['cgst']/100) : 0;
                $company['sgst'] = (!empty($decrypt_data['sgst']) && $key == 0) ? $decrypt_data['combo_coach_company_price'] * ($decrypt_data['sgst']/100) : 0;
				if($key == 0)
				{
					$company['member_commission'] = ($member_commission * $decrypt_data['combo_coach_company_price'])/100;
					$company['admin_commission'] = ($admin_commission * $decrypt_data['combo_coach_company_price'])/100;
				}
				
              }
			  else if(!empty($decrypt_data['selected_companies_price']))
              {
                $company['item_price'] = ($key == 0) ? $decrypt_data['interview_amount'] : 0;
                $company['combo_status'] = 1;
                $company['combo_type'] = $decrypt_data['company_count'].' Companies';
                $company['igst'] = (!empty($decrypt_data['igst']) && $key == 0) ? $decrypt_data['interview_amount'] * ($decrypt_data['igst']/100) : 0;
                $company['cgst'] = (!empty($decrypt_data['cgst']) && $key == 0) ? $decrypt_data['interview_amount'] * ($decrypt_data['cgst']/100) : 0;
                $company['sgst'] = (!empty($decrypt_data['sgst']) && $key == 0) ? $decrypt_data['interview_amount'] * ($decrypt_data['sgst']/100) : 0;
				if($key == 0)
				{
					$company['member_commission'] = ($member_commission * $decrypt_data['interview_amount'])/100;
			        $company['admin_commission'] = ($admin_commission * $decrypt_data['interview_amount'])/100;
				}
				else{
					$company['member_commission'] = 0;
					$company['admin_commission'] = 0;
				}
              ...
              else
              {
				...
				$company['member_commission'] = ($member_commission * $decrypt_data['interview_each_company_price'])/100;
			    $company['admin_commission'] = ($admin_commission * $decrypt_data['interview_each_company_price'])/100;
              }
              $company['item_id'] = $id;
    ...
    public function payment_responce(Request $request)
    {
        ...
        if($order_status=='Success')
        {
            ...
            */
            ...
            $transaction_qa_count =  $this->TransactionHistoryModel->select('id')->where('trans_id',$transaction_detail->id)->where('item_type','Interview_qa')->count();
			if($transaction_qa_count > 0)
			{
				$qa_parts = $this->MultiReferenceBookModel->where('interview_id',$transaction_detail->ref_interview_id)->get();
				if(count($qa_parts) > 0)
				{
					$parts = array();
					foreach($qa_parts as $key=>$part)
					{
						$items = array();
						$items['user_id'] = $transaction_detail->user_id;
						$items['transaction_id'] = $transaction_detail->id;
						$items['interview_id'] = $transaction_detail->ref_interview_id;
						$items['ref_book_id'] = $part->id;
						$items['created_at'] = date('Y-m-d H:i:s');
						$items['updated_at'] = date('Y-m-d H:i:s');
						
						$parts[] = $items;
					}
					DB::table('purchased_qa_parts')->insert($parts);
				}
			}
            if($transaction_detail->notification_request == 1)
            {
                $notification = NotificationModel::where(['interview_id'=>$transaction_detail->ref_interview_id, 'user_id'=>$transaction_detail->user_id])->update(['booked_date'=>date('Y-m-d H:i:s'),'ticket_unique_id'=>$transaction_detail->ticket_unique_id]);
            }

...
*/
        ...
        if($obj_user) 
        {
          ...
          $arr_data=[];
			...
			$arr_data['education_city']        = $request->input('city_id');
			$arr_data['education_state']        = $request->input('state_id');
			$arr_data['education_country_id']        = $request->input('country_id');
    ...
    {     /*dd($request->all());*/
		...
		$data['end_month']     = $request->input('current_end_month');
		$data['end_year']      = $request->input('current_end_year');
		$data['designation']   = $request->input('current_designation');
		$data['company_name']  = $request->input('employer_name');
		$data['member_id']     = $member_id;
		$data['employer_type'] = "current";
		$data['country'] = $request->input('company_country_id');		
		$data['state'] = $request->input('company_state_id');		
		$data['city'] = $request->input('company_city_id');
		$data['description'] = $request->input('current_description');
		//dd($data);
		...
		if(isset($previous_designation) && isset($previous_company_name) && isset($previous_start_month) && isset($previous_start_year) && isset($previous_end_month) && isset($previous_end_year))
		{
			...
			{
				$previous_data[$key]['description'] = $desc;
			}
			
			foreach($city as $key => $cty) 
			{
				$previous_data[$key]['city'] = $cty;
			}
			
			foreach($country as $key => $cnt) 
			{
				$previous_data[$key]['country'] = $cnt;
			}
			
			foreach($state as $key => $st) 
			{
				$previous_data[$key]['state'] = $st;
			}
        ...
        $arr_member_data['current_work_location']  = $data['city'];
        $arr_member_data['employment_country_id']  = $data['country'];
      ...
      $arr_rules['about_member']      = "required";
        ...
        $arr_member_data['inqa']               = $request->input('inqa');
         $arr_member_data['giveresume']               = $request->input('giveresume');
          $arr_member_data['collectinterview']               = $request->input('collectinterview');
           $arr_member_data['workexp']               = $request->input('workexp');
            $arr_member_data['accactive']               = $request->input('accactive');
      ...
      $user = Sentinel::findById($user_id);
        ...
        return redirect(url('/member/register'));
    ...
    public function user_store(Request $request)
    {
        ...
        if($obj_user) 
        {
          ...
          $arr_data=[];
            ...
            $arr_data['current_work_location'] = $request->input('city_id');
          ...
          if($check_authentication)
          {
               /* check if the user has the specified role */
                if(Sentinel::inRole('member'))
                {
                    ...
                    {  
						$log_array = array();
						$log_array['user_id'] = $user->id;
						$log_array['ip_address'] = $this->get_client_ip();
						$log_array['created_at'] = date('Y-m-d H:i:s');
						$log_array['updated_at'] = date('Y-m-d H:i:s');
						DB::table('log_users')->insert($log_array);
						
                        Session::put('logged_in','member');
                        Flash::success("You Logged In Successfully.");
                        if($request->input('preview-page'))
                        {
                            return redirect()->back();
                        }
                        return redirect('/member/upload_history');
                    }
                    else
                    { 
        ...
        if($activation) /* if activation entry there  in database*/
        {
            ...
            /* complete an activation process*/
            {
                ...
                if($tmp_user)
                {
                    ...
                    $tmp_user->activated_at = date('Y-m-d : H:i:s');
                    $tmp_user->save(); 
                }
  ...
  public function email_verification(Request $request)
  {
   ...
   public function getStates(Request $request) {
        ...
        $states = $this->StateModel->where(['is_active'=>1, 'country_id'=>$country])  
        ...
        if(isset($states))
        {
            ...
            $selected = ($state == $val->id) ? 'selected="selected"' : '';
            $option .= '<option value="' . $val->id . '" ' . $selected . '>' . ucfirst($val->state_name) . '</option>';
            }
        }
    ...
    public function getCities(Request $request) {
        ...
        $cities = $this->CityModel->where(['is_active'=>1, 'state_id'=>$state])  
        ...
        if(isset($cities))
        {
            foreach ($cities as $key => $val) {
                $selected = ($city == $val->city_id) ? 'selected="selected"' : '';
                $option .= '<option value="' . $val->city_id . '" ' . $selected . '>' . ucfirst($val->city_name) . '</option>';
            }
        }
        return response()->json($option);
    }
	...
	public function getCountryPhoneCode(Request $request) {
        $country = $request->input('country_id');
		$countryInfo = $this->CountryModel->where(['is_active'=>1, 'id'=>$country])  
                                      ->first();
 ...
 public function getcountrycodename(Request $request) {
        $country = $request->input('country_id');
 $country=str_replace('-',' ',$country);
        $states = $this->CountryModel->where('country_name','LIKE',"{$country}%")  
        ...
        if(isset($states))
        {
            ...
            $option .= '<p class="searchp" id="'. $val->id.'-'.ucfirst($val->country_name) .'-'.($val->phonecode) .'" >' . ucfirst($val->country_name) . '</p>';
            }
        }
     ...
     public function getstatecodename(Request $request) {
        $state_id = $request->input('state_id');
            $country = $request->input('country_id');


 $state_id=str_replace('-',' ',$state_id);
        $states = $this->StateModel->where('state_name','LIKE',"{$state_id}%")  
        ...
        if(isset($states))
        {
            ...
            $option .= '<p class="searchpstate" id="'. $val->id.'-'.ucfirst($val->state_name) .'" >' . ucfirst($val->state_name) . '</p>';
            }
        }
  ...
  public function autosearchboxcity(Request $request) {
        $state_id = $request->input('state_id');
            $country = $request->input('country_id');


 $state_id=str_replace('-',' ',$state_id);
        $states = $this->CityModel->where('city_name','LIKE',"{$state_id}%")  
                                      ->where('state_id','=',$country)    
                                      ->orderBy('city_name','asc')  
                                      ->get();
        $option = '';
        if(isset($states))
        {
            ...
            $option .= '<p class="searchpcity" id="'. $val->city_id.'-'.ucfirst($val->city_name) .'" >' . ucfirst($val->city_name) . '</p>';
            }
        }

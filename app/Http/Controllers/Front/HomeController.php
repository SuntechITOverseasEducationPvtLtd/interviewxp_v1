...
use App\Models\CareersaveModel;
...
class HomeController extends Controller
{
    ...
    public function __construct(MemberInterviewModel $memberinterview,
                                SkillsModel $skills, CareersModel $Careerspost,
                                ...
                                CareersaveModel $careersave_model,
                                ...
                                )
    {
        ...
        $this->CareersaveModel         = $careersave_model;
        ...
        $accountSettingsObj = $this->AccountSettingModel->where('id',1)->first();
        $this->deactivate_company_tab = $accountSettingsObj->deactivate_company_tab;
   ...
   public function index()
    {   
        $obj_interview = $this->MemberInterviewModel->join('users','users.id','=','member_interview.user_id')
                              ...
                              ->where('member_interview.admin_approval',1)
                              ->where('member_interview.is_active',1)
							  ->where('users.admin_status','Approved')
                              ->where('users.is_active',1)                              
                              ->orderBy('member_interview.publish_date','desc')
							  ->select('users.*', 'member_interview.*', 'member_interview.id as id')
                              ->take(7)
                              ->get();
        $arr_interview=[];                                                    
        if($obj_interview)
        ...
        /*$arr_user_count = $this->BaseModel->whereHas('roles',function($query)
                                        ...
                                        ->count();*/
        $arr_user_count = $this->TransactionModel->from('transaction as t')->join('transaction_history as th','th.trans_id','=','t.id')->where(['t.payment_status'=>'paid'])->groupBy('user_id')->count();   
        ///dd($arr_user_count);                             
        // Register Member Count
        $arr_member_count = $this->BaseModel->where('admin_status', 'Approved')->whereHas('roles',function($query)
                                              {
                                                $query->where('slug','!=','admin');
        ...
        */
        ...
        $count_approve = $this->MemberInterviewModel->where('admin_approval', 1)->count();  

        $arr_interview_hours_count = $this->TransactionModel->from('transaction as t')->join('transaction_history as th','th.trans_id','=','t.id')->where(['t.payment_status'=>'paid', 'th.item_type'=>'Coach'])->count();

        $arr_interview_hours_count = 4 * $arr_interview_hours_count;     
        ...
        $this->arr_view_data['arr_interview_hours_count'] = $arr_interview_hours_count;
        $this->arr_view_data['PriceListModel']        = $this->PriceListModel;
        $this->arr_view_data['UserModel']        = $this->UserModel;
...
public function mailset()
{
	...
	public function searchskill(Request $request)
    {
		...
		$query_search = $this->MemberInterviewModel->join('users','users.id','=','member_interview.user_id')->leftJoin('interview_detail as idl','member_interview.id','=','idl.interview_id')->leftJoin('skills','member_interview.skill_id','=','skills.id')->leftJoin('company_master as cmp','idl.company_id','=','cmp.company_id')
                              ->with(['user_purchase_details'=>function($query)
                              {
                                $query->where('payment_status','paid');
                              },'average_rating'=>function($query)
                              ...
                              ->where('member_interview.is_active',1)
							  ->where('users.admin_status','Approved')
							  ->where('users.company_qa_tab',1)
                              ->where('users.is_active',1)
                              ->where('skills.is_active',1)
                              ->where('cmp.is_active',1)
							  ->where(function($query) use ($search,$searchArray){
									$query->whereIn('member_interview.experience_level',$searchArray);
									$query->orWhereIn('idl.company_location',$searchArray);
									$query->orWhereIn('skills.skill_name',$searchArray);
									$query->orWhereIn('cmp.company_name',$searchArray);
									$query->orWhereIn('users.first_name',$searchArray);
									$query->orWhereIn('users.last_name',$searchArray);
								});  												
		foreach($searchArray as $word)
		{
			$query_search->orWhere(function($query) use ($word){
				$query->where('member_interview.experience_level','like',$word);
				$query->orWhere('idl.company_location','like','%'.$word.'%');
				$query->orWhere('skills.skill_name','like','%'.$word.'%');
				$query->orWhere('cmp.company_name','like','%'.$word.'%');
				$query->orWhere('users.first_name','like','%'.$word.'%');
				$query->orWhere('users.last_name','like','%'.$word.'%');
			});
		}		
														
														
		$obj_interview = $query_search->groupBy('member_interview.id')
		->orderBy('member_interview.publish_date','desc')
		->select('users.*', 'member_interview.*', 'member_interview.id as id')
		->get();
        //dd($obj_interview);                                            
    ...
    public function searchskillOld(Request $request)
    {   
        $arr_interview = [];
	...
	public function getUserReviews(Request $request)
    {
			...
			$interview_id = $request->input('interview_id');
			 $arr_review_rating = $this->ReviewRatingModel->where('interview_id',$interview_id)
        ...
        if($obj_interview)
        {
			...
			return view('front.userreviews', compact('arr_review_rating','arr_interview'), ['page'=>$page, 'interview_id'=>$interview_id]);			
		}
	}
...
public function learnlist()
    {
       ...
       $this->arr_view_data['module_title'] = 'My Wishlist';
...
$obj_interview = $this->MemberInterviewModel
                             ...
                             ->join('tbl_learnlist', 'member_interview.id', '=', 'tbl_learnlist.interviewes_id')
                              ->where('tbl_learnlist.useres_id',$this->user_id)
                              ->where('admin_approval',1)
                              ->where('is_active',1)
                              ->orderBy('publish_date','desc')
                              ->take(7)
        ...
        */
        ...
        return view('front.user.learnlist',$this->arr_view_data);
...
$obj_interview = $this->MemberInterviewModel
        ...
        */
        ...
        return view('front.member.learnlist',$this->arr_view_data);
...
public function alsobought($skillids,$interview_idsd)
    {
       $arr_data = [];
...
$obj_interview = $this->MemberInterviewModel->join('users','users.id','=','member_interview.user_id')
                              ...
                              ->where('member_interview.admin_approval',1)
                              ->where('member_interview.skill_id','=',$skillids)
                              ->where('member_interview.id','!=',$interview_idsd)
                              ->where('member_interview.is_active',1)
							  ->where('users.admin_status','Approved')
                              ->where('users.is_active',1)                              
                              ->orderBy('member_interview.publish_date','desc')
       ...
       return view('front.alsobought',$this->arr_view_data);
 ...
 public function learnlistsave(Request $request) {
      
        $interview_id = $request->input('interview_id');
        $learlist_data              = [];
        $learlist_data['useres_id'] = $this->user_id;
        $learlist_data['interviewes_id'] = $interview_id;
       
        

	 $is_duplicate =$this->CareersaveModel->where('useres_id',$this->user_id)->where('interviewes_id',$interview_id)->count();
if($is_duplicate==0){

        $learliststore = $this->CareersaveModel->create($learlist_data);
        ...
        public function deletelearnlist($enc_id = FALSE)
    {
$id=base64_decode($enc_id);
       ...
       $delete_success = $this->CareersaveModel->where('ids',$id)->delete();
 ...
 public function getreviwratingshow(Request $request) {
        $reviwidset_id = $request->input('reviwidset_id');
        $interview_id = $request->input('interview_id');

     
       
       $reviwidset_idf=$reviwidset_id+10;
       
          $obj_review_rating = $this->ReviewRatingModel->where('interview_id',$interview_id)
                                                     ->where('approve_status','user')
                                                     ->orWhere('approve_status','both')
                                                     ->with(['user_details'])   
                                                     ->limit($reviwidset_id)->offset(10)->get();
                                                     
                                                     
                                                     $countvalues = $this->ReviewRatingModel->where('interview_id',$interview_id)
                                                     ->where('approve_status','user')
                                                     ->orWhere('approve_status','both')
                                                     ->with(['user_details'])   
                                                     ->count();
        ...
        { 
            $arr_review_rating = $obj_review_rating;
         ...
         return view('front.userreviewsshow',['page'=>$countvalues,'startid'=>$reviwidset_idf, 'arr_review_rating' =>$arr_review_rating]);                                           
    }
    
    
    public function getreviwrating(Request $request) {
        $reviwidset_id = $request->input('reviwidset_id');
        $interview_id = $request->input('interview_id');
          ...
          $obj_review_rating = $this->ReviewRatingModel->where('interview_id',$interview_id)
                                                     ->where('approve_status','user')
                                                     ->where('review_star',$reviwidset_id)
                                                     ->orWhere('approve_status','both')
                                                     ->with(['user_details'])   
                                                     ->paginate(25);
            $count_rating   = 0;
        ...
        { 
         ...
         return view('front.userreviewss',['page'=>1,'startid'=>$reviwidset_id, 'arr_review_rating' =>$arr_review_rating]);                                           
    }
    ...
    public function interview_details($interview_id)
    {
        ...
        { 
            ...
            $average_rating = ($count_rating/$count_reviewed_users); 
        }                                         
        ...
        $obj_interview = $this->MemberInterviewModel->where('id',$interview_id)
                                                    ...
                                                    ,'reference_book_details'=> function ($reference_book)
                                                    {
                                                        $reference_book
                                                        ->where('approve_status',1); 
                                                    },'memberdetails','user_purchase_details','user_details'])
                                                    ->first();
     ...
     $company_details = array_map("unserialize", array_unique(array_map("serialize", $arr_final_result))); */
        ...
        $company_details = InterviewDetailModel::from('interview_detail as i')->join('company_master as c','c.company_id','=','i.company_id')->where(['i.interview_id'=>$interview_id, 'i.approve_status'=>'1'])->where('file_extension', '!=' ,'youtube')->select('i.*','c.*','i.company_location')->get();
        ...
        $obj_training_schedule = $this->TrainingSchedulesModel->where(['skill_id'=>$skill_id, 'member_id'=>$member_id, 'status'=>'Live'])
            ->where(DB::raw("DATE_FORMAT(created_at,'%Y-%m-%d')"),'<=','DATE_ADD(date, INTERVAL 4 DAY)')
            ->where(DB::raw("DATEDIFF(now(),date)") ,'<=', 4)
        ...
        $this->arr_view_data['auth_user_id']                 = isset($this->user_id) ? $this->user_id : '';
        $this->arr_view_data['user_id']                      = $user_id;
        $this->arr_view_data['member_id']                    = $member_id;
        $this->arr_view_data['interview_id']                 = $interview_id;
        $this->arr_view_data['skill_name']                   = $skill_name;
        $this->arr_view_data['deactivate_company_tab']       = $this->deactivate_company_tab;
        ...
        $this->arr_view_data['notificationModel']               = $this->NotificationModel;
        /*dd($arr_interview);*/
        return view('front.member.interview-detail',$this->arr_view_data);
    }

    public function realtimeTickets(Request $request){
            ...
            $ticketslist = DB::table('member_real_time_experience')
                            ->select('*')
                            ->where(['interview_id'=>$interview_id, 'user_id'=>$user_id, 'member_id'=>$member_id, 'skill_id'=>$skill_id, 'experience_level'=>$experience_level, 'approve_status'=>1])
                            ->orderBy('issue_title','DESC')
                            ->paginate(10);

            if ($request->ajax()) {
                return view('front.rtresult', compact('ticketslist'), ['page'=>$page]);
            }
    }

    public function getUserRealtimeTickets(Request $request){
            ...
            $curr_checked_values = $request->input('checked_values');

            $checked_values = Session::get('checked_values');
            $checked_values[$prevPage] = $curr_checked_values; 
            Session::put('checked_values',$checked_values);

            $tickets = DB::table('member_real_time_experience')
                            ->select('*')
                            ->where(['interview_id'=>$interview_id, 'user_id'=>$user_id, 'skill_id'=>$skill_id, 'experience_level'=>$experience_level, 'approve_status'=>1])
                            ->orderBy('issue_title','DESC')
                            ->paginate(15);

            if ($request->ajax()) {
                return view('front.userrtresult', compact('tickets'), ['page'=>$page,'limit'=>$limit]);
            }
    }     
    ...
    public function careers()
    {
        ...
        $obj_career = $this->CareersModel->where('status','3')->orderBy('id', 'DESC')->get();
        {
    ...
    public function careers_form($career_slug)
    {
         ...
         $id = base64_decode($career_slug);
         
         $obj_career = $this->CareersModel->where('id',$id)->first();
        {
            $arr_career = $obj_career->toArray();
        }
     ...
     public function store_careers(Request $request)
    {
        ...
        $arr_member_data['cnn']                    = $request->input('company_namen');
        $arr_member_data['dn']                    = $request->input('designationn');
        $arr_member_data['smn']                    = $request->input('duration_month_startn');
        $arr_member_data['syn']                    = $request->input('duration_year_startn');
        $arr_member_data['emn']                    = $request->input('duration_month_endn');
        $arr_member_data['eyn']                    = $request->input('duration_year_endn');
        $arr_member_data['asn']                    = $request->input('annual_saln');
        $arr_member_data['cnnn']                    = $request->input('company_namenn');
        $arr_member_data['dnn']                    = $request->input('designationnn');
        $arr_member_data['smnn']                    = $request->input('duration_month_startnn');
        $arr_member_data['synn']                    = $request->input('duration_year_startnn');
        $arr_member_data['emnn']                    = $request->input('duration_month_endnn');
        $arr_member_data['eynn']                    = $request->input('duration_year_endnn');
        $arr_member_data['asnn']                    = $request->input('annual_salnn');
        $arr_member_data['postid']                    = $request->input('postid');
    ...
    public function function_view_all()
    {
        ...
        {
            $obj_category = $this->CategoryModel->paginate(50);
			$this->arr_view_data['result'] = $obj_category;
			$this->arr_view_data['module_name'] = 'Categories';
        }

        if(\Request::segment(1)=='all_company')
        {
            $obj_company = $this->CompanyModel->paginate(50);
			$this->arr_view_data['result'] = $obj_company;
			$this->arr_view_data['module_name'] = 'Companies';            
        }

        if(\Request::segment(1)=='all_qualification')
        {
            $obj_qualification = $this->QualificationModel->paginate(50);
			$this->arr_view_data['result'] = $obj_qualification;
			$this->arr_view_data['module_name'] = 'Qualifications';
        }
        if(\Request::segment(1)=='all_specialization')
        {
            $obj_specialization = $this->SpecializationModel->groupBy('specialization_name')
                              ->orderBy('specialization_name')->paginate(50);
			$this->arr_view_data['result'] = $obj_specialization;
        ...
        {
            $obj_skills = $this->SkillsModel->paginate(50);
            $this->arr_view_data['result'] = $obj_skills;
    ...
    public function function_search_data($enc_id)
    {
       if(\Request::segment(1)=='category')
        ...
        {
            $id = base64_decode($enc_id);
            $obj_specializations = $this->SpecializationModel->select('id')->where('specialization_name',$id)->get();
            $arr_specializations = [];
            if($obj_specializations)
            {
                foreach($obj_specializations as $value)
                {
                    $arr_specializations[] = $value->id;
                }
                //$arr_specializations = $obj_specializations->toArray();
                //$specializations = implode(',',$arr_specializations);
            }
            //$specializations = implode(',',$arr_specializations);
            $obj_interview = $this->MemberInterviewModel
                              ->with(['user_purchase_details',
                              ...
                              ->where('admin_approval',1)
                              ->where('is_active',1)
                              ->whereIn('specialization_id',$arr_specializations)
                              ->orderBy('publish_date', 'DESC')
                              ->get();
            $arr_interview=[];                                                    
            if($obj_interview)
        ...
        {
            $id = base64_decode($enc_id);
            $skills = $this->SkillsModel->where('skill_name', 'like', '%' . $id . '%')->get();
            $arr_skills = [];
            if($skills)
            {
        ...
        if(\Request::segment(1)=='company')
        {
            ...
            $companies = $this->CompanyModel->where('company_name', 'like', '%' . $id . '%')->get();
            $arr_companies = [];
            if($companies)
            {
                foreach($companies as $value)
                {
                    $arr_companies[] = $value->company_id;
                }
                $arr_skills = [];
                $obj_company_interview_detail = $this->InterviewDetailModel->whereIn('company_id',$arr_companies)
                                  ->groupBy(['skill_id','experience_level'])->get(['interview_id']);
                if(count($obj_company_interview_detail) > 0)
                {
                    foreach($obj_company_interview_detail as $value)
                    {
                        $arr_interviews[] = $value->interview_id;
                    }
                }   
                $obj_interview = $this->MemberInterviewModel
                                          ->with(['user_purchase_details',
    ...
    public function get_specialization($qualification_id)
    {
     ...
     public function validate_current_enrollment()
    {
        $schedule_id = $_GET['schedule_id'];
        $skill_id = $_GET['skill_id'];
        $user_id = $_GET['user_id'];

        $obj_schedule = $this->TrainingSchedulesModel->where('id',$schedule_id)->first();
        if(empty($obj_schedule))
        {
            return response()->json(['status'=>false, 'msg'=>'No schedules found']);
        }

        if(isset($this->user_id))
        {

            $isScheduleAlreadyBooked = DB::table('transaction')
            ...
            if($isScheduleAlreadyBooked > 0)
            {
                return response()->json(['status'=>false, 'msg'=>'You already booked this online class']);
            } 
        }           

        $start_date = $obj_schedule->created_at;
        $max_allowed = $obj_schedule->max_allowed;
        $closing_days = config('constants.CLOSE_SCHEDULE_DAYS');


        $newEndDate = strtotime("+".$closing_days." days", strtotime($obj_schedule->date));
        ...
        if($max_allowed == $scheduleCount){

            return response()->json(['status'=>false, 'msg'=>'Current Schedule has no attendees left!']);
        } 

        return response()->json(['status'=>true, 'closing_days'=>$closing_days]);   
    }

    public function learning_preview($topic_name, $interview_id)
    {
        $topic_name = str_replace("-"," ",$topic_name);
        $interview_id = base64_decode($interview_id);
        /*$previewData = DB::select( DB::raw("SELECT * FROM multi_reference_book WHERE interview_id = '".$interview_id."' AND topic_name = '".$topic_name."'") );*/

        $previewData = DB::select( DB::raw("SELECT * FROM multi_reference_book mr INNER JOIN member_interview i ON i.id = mr.interview_id INNER JOIN skills s ON s.id = i.skill_id WHERE mr.interview_id = '".$interview_id."' AND mr.approve_status='1' group by mr.topic_name") );

                
        $this->arr_view_data['topic_name'] = $topic_name;
        $this->arr_view_data['skill_name'] = $previewData[0]->skill_name;
        $this->arr_view_data['interview_id']    = $interview_id;
        $this->arr_view_data['previewData']    = $previewData;
        return view('front.learning_preview',$this->arr_view_data);
    }

    public function preview_company($company_id, $interview_id)
    {
        $company_id = base64_decode($company_id);
        $interview_id = base64_decode($interview_id);
        
        $previewData = DB::select( DB::raw("SELECT * FROM interview_detail id INNER JOIN member_interview i ON i.id = id.interview_id INNER JOIN skills s ON s.id = i.skill_id WHERE id.interview_id = '".$interview_id."' AND id.approve_status='1'  AND company_id = '".$company_id."' AND file_extension != 'youtube' ") );

        
        $this->arr_view_data['company_id'] = $company_id;
        $this->arr_view_data['skill_name'] = $previewData[0]->skill_name;
        $this->arr_view_data['interview_id']    = $interview_id;
        $this->arr_view_data['previewData']    = $previewData;
        return view('front.company_learning_preview',$this->arr_view_data);
    }

     public function preview_real_issues($skill_id, $member_id, $user_id, $interview_id, $experience_level)
    {
        $skill_id = base64_decode($skill_id);
        $member_id = base64_decode($member_id);
        $user_id = base64_decode($user_id);
        $interview_id = base64_decode($interview_id);
        $experience_level = base64_decode($experience_level);
        
        $previewData = DB::select( DB::raw("SELECT * FROM `member_real_time_experience` INNER JOIN skills ON skills.id = member_real_time_experience.skill_id WHERE `interview_id` = ".$interview_id." AND `user_id` = ".$user_id." AND `member_id` = ".$member_id." AND `skill_id` = ".$skill_id." AND `experience_level` = '".$experience_level."' AND `approve_status` = '1' ORDER BY `issue_title` DESC limit 0,10") );
                                         

        $this->arr_view_data['topic_name'] = $skill_id;
        $this->arr_view_data['skill_name'] = $previewData[0]->skill_name;
        $this->arr_view_data['interview_id']    = $interview_id;
        $this->arr_view_data['previewData']    = $previewData;
        return view('front.real_issues_learning_preview',$this->arr_view_data);
    }

   public function notifyCoach(Request $request)
    {
        if($request->ajax()){
            
            $arr_noti  = [];
            $arr_noti['user_id'] = $request->input('user_id');
            $arr_noti['interview_id'] = $request->input('interview_id');
            
            $notifications = $this->NotificationModel->where(['interview_id'=>$arr_noti['interview_id'], 'user_id'=>$arr_noti['user_id']])->first();
            if(count($notifications) > 0)
            {
                $arr_noti['status'] = $request->input('coach_notify');
                $arr_noti['message'] = 'Coach Availability Notification : '.$request->input('message');
                $arr_noti['update_at'] = date('Y-m-d H:i:s');
                $notifications->update($arr_noti);
            }
            else{
                $arr_noti['status'] = $request->input('coach_notify');
                $arr_noti['message'] = 'Coach Availability Notification : '.$request->input('message');
            ...
            if($arr_noti['status'] == 1)
            {
                //$obj_arr_data = $this->BaseModel->where('id',$arr_noti['user_id'])->first();
                //$email_id = $obj_arr_data->email;
                //$email_id = 'lakshman.garikapati@gmail.com';
               // $data['user_id'] = $arr_noti['user_id'];
                //$data['name'] = $obj_arr_data->first_name;
                //$data['email_id'] = $email_id;
                //$project_name = config('app.project.name');
                //$mail_from = $this->mail_from;

                /*Mail::send('front.email.notify_coach', $data, function ($message) use ($email_id,$mail_from,$project_name) {
                          $message->from($mail_from, $project_name);
                          $message->subject($project_name.': Coach Availability Notification');
                          $message->to($email_id);
                      });*/
    ...
    {
        $notifications = $this->NotificationModel->where(['status'=>1])->where('interview_id','!=',0)->get();
        foreach($notifications as $obj)
        {
            $obj_arr_data = $this->BaseModel->where('id',$obj->user_id)->first();
            $email_id = $obj_arr_data->email;
            //$email_id = 'lakshman.garikapati@gmail.com';
            ...
            Mail::send('front.email.coach_availability', $data, function ($message) use ($email_id,$mail_from,$project_name) {
                      $message->from($mail_from, $project_name);
                      $message->subject($project_name.': Interview Coach is Available');
                      $message->to($email_id);
                  });
        }
    }
    ...
    public function getCompanies(Request $request)
    {
        if($request->ajax()){           
            ...
            $results = '';
            $obj_company = $this->CompanyModel->where('company_name', 'like', '%' . $term . '%')->limit(13)->get();
            foreach ($obj_company as $value)
            {
            $results[] = [ 'id' => $value->company_id, 'value' => $value->company_name];
            }
    ...
    {
        $id = $_GET['id'];
        $filename = public_path() . "/uploads/refrence_book/$id";
        $headers = array(
    ...
    {
        $sql ='select count(*) as cnt,interview_id FROM `transaction` as t INNER JOIN `transaction_history` as th ON t.id = th.trans_id  where th.`item_type`="Coach" AND t.payment_status = "paid" AND t.ticket_unique_id NOT IN (SELECT unique_id FROM `review_rating`) group by t.`ref_interview_id` having cnt < 10';
        ...
        if(count($result) > 0)
        {
            foreach ($result as $key => $value) {
                ...
                $notifications = $this->NotificationModel->join('users','users.id','=','notification.user_id')->select(['user_id','first_name','last_name','email','interview_id'])->where(['interview_id'=>$interview_id, 'notification.status'=>1,'email_sent_status'=>0])->get();
                //dd($interview_id);
                foreach ($notifications as $key => $user) {
                    //$email_id = 'lakshman.garikapati@gmail.com';
                    $email_id = $user->email;
                    $data['name'] = ucfirst($user->first_name);
                    $data['email_id'] = $email_id;
                    $data['booking_link'] = url('/').'/interview_details/'.base64_encode($user->interview_id).'?ref=notification';
                    //dd($data['booking_link']);
                    $project_name = config('app.project.name');
                    $mail_from = $this->mail_from;

                    $this->NotificationModel->where(['interview_id'=>$user->interview_id, 'user_id'=>$user->user_id])->update(['email_sent_date'=>date('Y-m-d H:i:s'),'email_sent_status'=>1]); 
                   Mail::send('front.email.email_notifications', $data, function ($message) use ($email_id,$mail_from,$project_name) {
                          $message->from($mail_from, $project_name);
                          $message->subject($project_name.':Coach Availability');
                          $message->to($email_id);
                      });

...
class HomeController extends Controller
{
    ...
    public function interview_details($interview_id)
    {
        ...
        $company_details = InterviewDetailModel::from('interview_detail as i')->join('company_master as c','c.company_id','=','i.company_id')->where(['i.interview_id'=>$interview_id, 'i.approve_status'=>'1'])->select('i.*','c.*','i.company_location')->get();
    ...
    public function preview_company($company_id, $interview_id)
    {
        ...
        $previewData = DB::select( DB::raw("SELECT * FROM interview_detail id INNER JOIN member_interview i ON i.id = id.interview_id INNER JOIN skills s ON s.id = i.skill_id WHERE id.interview_id = '".$interview_id."' AND id.approve_status='1'  AND company_id = '".$company_id."'") );
    ...
    public function notifyCoach(Request $request)
    {
        if($request->ajax()){
            ...
            if(count($notifications) > 0)
            {
                ...
                $arr_noti['updated_at'] = date('Y-m-d H:i:s');

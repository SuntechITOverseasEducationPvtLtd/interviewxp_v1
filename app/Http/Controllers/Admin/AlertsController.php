...
class AlertsController extends Controller
{
    public function __construct(UserModel $user,
                                ...
                                UserDetailModel $user_details  
                                )
    {
    	$this->UserModel            = $user;
        $this->UserAlertsModel      = $user_alerts;
        $this->BaseModel            = $this->UserAlertsModel;

        $this->UserDetailModel      = $user_details;
        $this->StateModel           = $state;
        $this->ActivityLogsModel    = $activity_logs;
        
        $this->arr_view_data        = [];
        $this->admin_url_path       = url(config('app.project.admin_panel_slug'));
        $this->module_url_path      = $this->admin_url_path."/alerts";
        $this->module_view_folder   = "admin.user_alerts";
        $this->module_title         = "Subscribed For Alerts";
        /*For Activity log*/
    ...
    {
        $arr_data = [];
        $obj_data = $this->BaseModel->with(['skills','user_details'])->get();
        {
    ...
    {    
        ...
        $arr_data = [];
        $obj_data = $this->BaseModel->where('alert_id',$id)->with('skills','user_details')->first();
        if($obj_data)
        {
            $arr_data = $obj_data->toArray();
        }
    ...
    public function store(Request $request)
    {
        ...
        $does_exists = $this->BaseModel
                            ->where('category_name','=',$request->input('category_name'))
                            ->count();   
        if($does_exists)
        {
        ...
        $arr_user_data                  = [];
        $arr_user_data['category_name'] = $request->input('category_name');
    ...
    {
        $entity = $this->BaseModel->where('alert_id',$id)->first();
        if($entity)
        {
            ...
            return $this->BaseModel->where('alert_id',$id)->update(['is_active'=>1]);
        }
    ...
    public function perform_deactivate($id)
    {
        ...
        if($entity)
        {
            ...
            return $this->BaseModel->where('alert_id',$id)->update(['is_active'=>0]);
        }
        return FALSE;
    }
    ...
    {
       $delete_success = $this->BaseModel->where('alert_id',$id)->delete();

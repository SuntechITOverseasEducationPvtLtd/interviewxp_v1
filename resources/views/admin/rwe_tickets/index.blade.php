         ...
         <div class="btn-toolbar pull-right clearfix">
            ...
            <div class="btn-group"> 
               <a class="btn btn-circle btn-to-success btn-bordered btn-fill show-tooltip call_loader" 
                  title="Refresh" 
                  href="{{ $module_url_path.'/member_rwe_tickets/'.base64_encode($arr_data[0]['member_id']) }}"
                  style="text-decoration:none;">
            ...
            <table class="table table-advance"  id="table_module" >
               ...
               <tbody>
                  ...
                  <tr>
                     ...
                     <td > {{ $data['member_personal_details']['first_name'] or 'NA'}} {{ $data['member_personal_details']['last_name'] or 'NA'}}  </td>
                     <td>{{ isset($data['issue_title']) && $data['issue_title']!=""?$data['issue_title']:'NA'}}</td>
                     <td>  {{isset($data['skill_name']) && $data['skill_name']!=""?$data['skill_name']:'NA'}} </td>
                     <td>{{ isset($data['experience_level']) && $data['experience_level']!=""?$data['experience_level']:'NA' }}</td>
                     <?php 
                        $solution=str_limit(ucfirst($data['solution']),$limit = 90, $end='...');
                        ?>
                     <!--  <td> {{isset($solution) && $solution!=""?$solution:'NA'}} </td> -->
                     <td style="width:30%">
                        <div style="height:50px; overflow:auto">
                           {{isset($data['solution']) && $data['solution']!=""?$data['solution']:'NA'}} 
                        </div>
                     </td>
                     <td>
                        @if($data['attachment']!='')<a href="{{$referencebook_public_path.$data['attachment']}}" download='' ><i class="fa fa-download" aria-hidden="true"></i></a>
                        @else
                        NA
                        @endif
                     </td>
...
<script>
   function approve_change(val,id) {
         ...
         {            
               ...
               var success_link  = "{{url('/')}}/admin/rwe_tickets/member_rwe_tickets/"+member_id;
               //alert(approve_value);
               $.ajax({
               url: "{{url('/')}}/admin/rwe_tickets/approve_change",
               type: "POST",
               async: false,

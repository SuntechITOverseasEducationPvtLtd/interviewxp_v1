														 ...
														 <table class="datatable table table-striped table-bordered table table-advance" >
																<thead>
																   ...
																   <tr class="top-strip-table">
																	  <td style="font-family: 'ubuntumedium',sans-serif;font-size: 13px;">S.No <i class="fa fa-fw fa-sort"></i></td>
																	  	  <td style="font-family: 'ubuntumedium',sans-serif;font-size: 13px;">
																	  	      <div style="width:30%; float:left; text-align:left">Tickets, Tasks, Etc.,  <i class="fa fa-fw fa-sort"></i></div> <div style="width:20%; float:left; text-align:left">File Size <i class="fa fa-fw fa-sort"></i></div>
																	  	      <div style="width:30%; float:left; text-align:left">Date & Time Uploaded <i class="fa fa-fw fa-sort"></i></div>  <div style="width:20%; float:left; text-align:left">Status <i class="fa fa-fw fa-sort"></i></div> </td>
																   ...
																   <?php
																     ...
																     $results = DB::select( DB::raw("SELECT * FROM member_real_time_experience WHERE interview_id = '$arr_skill' AND file_extension != 'youtube' group by issue_title ORDER BY `id` DESC") );
														...
														foreach ($results as $key=>$user) {
															...
															?>
																		<tr class="asgreen" >
																			...
																			<td  title="{{$user->issue_title}}" > &nbsp;&nbsp;{{$string}} <br>
																				  	<?php
																	   $results1 = DB::select( DB::raw("SELECT * FROM member_real_time_experience WHERE interview_id = '$arr_skill' AND issue_title = '".$user->issue_title."' AND file_extension != 'youtube'") );
...
if(isset($results1s) && count($results1s)>0) { echo '<div style="line-height: 23px;
  ...
  <img src="{{url('/')}}/images/icon-play.svg" class="img-zoom"  style=" float: none;"></a>
     ...
     <?php } ?>
							...
							<div style="width:21%; float:left"> <?php echo round($user1s->fileSize/60) ?> Min.</div>
							<div style="width:30%; float:left"> {{date('j M, Y, g:i A T',strtotime($user1s->created_at))}} 

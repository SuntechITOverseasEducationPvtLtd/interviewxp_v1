<p class="show-results">Showing {{($arr_review_rating->currentpage()-1)*$arr_review_rating->perpage()+1}} to {{($arr_review_rating->currentpage()-1) * $arr_review_rating->perpage() + $arr_review_rating->count()}}
of  {{$arr_review_rating->total()}} results (<?=($startid);?> Star) <a href="" class="user-icon" style="float:right"><i class="fa fa-remove" aria-hidden="true"></i></a></p>
